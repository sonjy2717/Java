...
상속(Inheritance)
	...
	클래스를 정의하는 것을 말한다. 부모 클래스를 자식 클래스가
	상속할 때는 extends 키워드를 사용한다.
 */

// 부모 클래스(상위 클래스) : 사람의 일반적인 형태를 추상화
class Man {
	// 멤버 변수는 private으로 정의했으므로 클래스 내부에서만 접근 가능
	private String name;
...
class BusinessMan extends Man {
	...
	public BusinessMan(String name, String company, String position) {
		/*
		상속받은 자식 클래스는 부모 클래스의 생성자를 호출하여
		부모 객체가 먼저 생성되도록 해야한다.
		super()는 부모 클래스의 생성자를 호출하는 역할을 한다.
		만약 super()를 기술하지 않으면 디폴트 생성자가 호출된다.
		 */
		super(name); // 부모 클래스의 매개 변수 1개인 생성자를 호출한다.
	...
	public void tellYourInfo() {
		...
		/*
		부모쪽의 멤버 변수 name은 private으로 선언되었으므로 상속 받은
