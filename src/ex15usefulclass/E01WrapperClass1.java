...
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		Integer n20Obj = new Integer(num2); // 디플리케이트 되었다.
		int result2 = n10Obj.intValue() + n20Obj.intValue();
		System.out.println("result2 = " + result2);
		
		Integer resultObj = new Integer(result2);
		System.out.println("결과 값을 byte형 값으로 변경: " +
				resultObj.byteValue());
		System.out.println("결과 값을 이진수로 변경: " +
				Integer.toBinaryString(result2));
		
		/*
		JDK1.5 이후 버전에서의 형태. 오토박싱 혹은 오토언박싱이
		지원되므로 기본자료형 변수를 선언하는 것과 동일하게
		기술할 수 있다.
		 */
		Integer numObj1 = 100;
		Integer numObj2 = 200;
		...
		/*
		Wrapper클래스를 통해 진법변환이나 해당 자료형의 최대/최소 값을
		알 수 있다.
		 */
		System.out.println("int형의 최대 값: " + Integer.MAX_VALUE);
		System.out.println("int형의 최소 값: " + Integer.MIN_VALUE);
		...
		그러므로 아래 메서드를 사용할 수 있다.
		
		toString() : 인스턴스 변수를 출력할때 참조 값이 아니라
			실제 저장된 값을 문자열로 반환해주는 메서드
		hashCode() : 객체를 구별하기 위해 참조 값을 고유한 정수 값으로
			반환해주는 메서드
		 */
